...
class target_finder(object):
    ...
    def __init__(self, cam: cv2.VideoCapture):
        ...
        self.frame = np.zeros(shape=(480, 640, 3))
        self.camera = cam
        self.color = 0
        self.cx = -1
        self.cy = -1
        ...
        # self.hsv_l = np.array((70, 50, 60))
        # self.hsv_h = np.array((80, 255, 255))
    ...
    def __find(self):
        while True:
            self.frame = self.__acquireImage()
            thresh = self.__preImageProcessing(self.frame)

            # if self.show:
            # cv2.imshow("Targets", thresh)
            # cv2.waitKey(1)

            targets = self.__findTargets(thresh)
            ...
            if len(rectangles) > 0:
                ...
                cv2.circle(self.frame, (cx, cy), 10, (255, 0, 255), 10)

                distance = calculateDistance(cy)
                heading = calculateAngle(cx)

                if 6 < distance < 7 or 14 < distance < 16:
                    cv2.rectangle(self.frame, rect_coords[0], rect_coords[1], (0, 255, 255), -1)
                elif 7 <= distance <= 14 and abs(distance * sin(pi * heading / 180)) < 0.8:
                    cv2.rectangle(self.frame, rect_coords[0], rect_coords[1], (0, 255, 0), -1)

                (text_w, text_h), _ = cv2.getTextSize("{:.1f}ft".format(distance), FONT, 1, 4)
                TEXT_PADDING = 5

                cv2.rectangle(self.frame, (0, 0), (TEXT_PADDING * 2 + text_w, TEXT_PADDING * 2 + text_h), (0, 0, 0), -1)
                cv2.putText(self.frame, "{:.2f}ft".format(distance), (0, 30), FONT, 1, (255, 255, 255), 4, cv2.LINE_AA)

                (angle_w, angle_h), _ = cv2.getTextSize("{:.1f} deg".format(heading), FONT, 1, 4)
                cv2.rectangle(self.frame, (w - angle_w - TEXT_PADDING, 0),
                              (w - angle_w - TEXT_PADDING, TEXT_PADDING * 2 + angle_h), (0, 0, 0), -1)
                cv2.putText(self.frame, "{:.1f} deg".format(heading), (w - angle_w, 30), FONT, 1, (255, 255, 255), 4,
                            cv2.LINE_AA)

                self.result = (True, distance, heading)
            else:
                cv2.rectangle(self.frame, rect_coords[0], rect_coords[1], (0, 0, 255), -1)
            # if self.show:
            # cv2.imshow("TargetCam", frame)
            # cv2.waitKey(1)

    def get_data(self):
        return self.result, self.frame
